...
// Create a complete audiobook pipeline from file upload
router.post('/create-from-upload', async (req, res) => {
    try {
        ...
        const tempFilePath = path.join(__dirname, '../../temp-uploads', uploadedFile.name);
        ...
        const job = {
            ...
            progress: 0,
            steps: {
                upload: { status: 'completed', message: 'File uploaded' },
        ...
        processUploadPipeline(jobId, tempFilePath, uploadedFile.name, { summarize: summarize === 'true', summaryStyle });

        res.json({
            jobId,
            status: 'started',
            message: `Upload pipeline started for "${uploadedFile.name}"`,
            trackingUrl: `/api/pipeline/status/${jobId}`,
            estimatedTime: job.estimatedTime
        });

    } catch (error) {
        logger.error('Upload pipeline creation error:', error);
        ...
        res.status(500).json({ error: 'Failed to start upload pipeline', message: error.message });
    }
});
